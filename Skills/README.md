...
| [documents/style-guidelines/](./documents/style-guidelines/) | Brand Style Guide Template | A fill-in-the-blank template for turning any organization's brand guide into an AI skill. Covers color palette, typography, spacing tokens, and component styles. |
...
| [operations/dossier/](./operations/dossier/) | Dossier | Renders a grid of subject profile cards or a single-subject one-pager briefing. Each subject has a header, key facts, tags, summary paragraph, and recent-activity bullets. Use for competitor profiles, account briefs, candidate summaries, site inventories, board bios, and partner overviews. Auto-selects grid mode (2–8 subjects) or one-pager mode (1 subject). |
| [operations/exec-report/](./operations/exec-report/) | Exec Report | Generates a polished, self-contained HTML executive report from SharePoint lists, CSVs, or a description. Interviews the user, determines narrative intent (achievement / problem / status), builds a validated JSON document, then renders it using a component library. Supports eight block types (hero sentence, KPI row, trend chart, bar list, compare strip, table, status grid, highlights) and four editorial palettes. |
...
| [operations/roadmap-timeline/](./operations/roadmap-timeline/) | Roadmap Timeline | Renders a horizontal swimlane roadmap or vertical milestone timeline as a self-contained HTML file. Computes all phase-block positions from dates as CSS percentages, renders status colors, today-line, and milestone diamonds. Use for project plans, product roadmaps, migration timelines, onboarding sequences, and run-of-show schedules. |
| [operations/scorecard-matrix/](./operations/scorecard-matrix/) | Scorecard Matrix | Renders a weighted heatmap scoring matrix: entities as rows, scored across weighted dimensions as columns, with computed totals, rank badges, and a winner callout. Domain-neutral — works for vendor selection, tool evaluation, candidate scoring, site-readiness grading, or competitive analysis. |
...
| [styling/style-blueprint/](./styling/style-blueprint/) | Style: Blueprint | Blueprint engineering-drawing style tokens for the list-styling skill. Monochromatic blue palette, double-ring rubber-stamp badges, ultra-compact two-line spec-sheet card. Overdue is the only red element. Use when the user says "blueprint," "technical drawing," "engineering," "schematic," or "spec sheet." |
...
| [writing/review-council/](./writing/review-council/) | Review Council | Convenes a council of expert AI personas to review, stress-test, and improve any document, idea, proposal, or plan. Works on any format — .docx, .xlsx, .pptx, .pdf, pasted text, or a verbal idea. Replaces generic feedback with structured multi-perspective debate and a clear verdict. |
